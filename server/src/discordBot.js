    ...
    switch (action) {
        case 'yes_button':
            ...
            try {
                ...
                const fields = originalEmbed.fields.map(field => {
                    ...
                    if (field.name === 'Attendees') {
                        ...
                        if (field.value === 'Currently no attendees') {
                            return { name: 'Attendees', value: interaction.user.username, inline: true };
                        }
                        return { name: 'Attendees', value: `${field.value}\n${interaction.user.username}`, inline: true };
                    }
                    if (field.name === 'NonAttendees') {
                        if (field.value.includes(`${interaction.user.username}`)) {
                            if (field.value.replace(`${interaction.user.username}`, '') === '') {
                                return { name: 'NonAttendees', value: 'Currently no non-attendees', inline: true };
                            }
                            return { name: 'NonAttendees', value: field.value.replace(`${interaction.user.username}`, ''), inline: true };
                        }
                    }
                    return field;
                });
                ...
                components[0].components[0].setCustomId(`yes_button/${swarmId}/${Date.now().toString()}`);
                components[0].components[1].setCustomId(`no_button/${swarmId}/${Date.now().toString()}`);
            ...
            const fields = originalEmbed.fields.map(field => {
                ...
                if (field.name === 'NonAttendees') {
                    ...
                    if (field.value === 'Currently no non-attendees') {
                        return { name: 'NonAttendees', value: `${interaction.user.username}`, inline: true };
                    }
                    return { name: 'NonAttendees', value: `${field.value}\n${interaction.user.username}`, inline: true };
                }
                if (field.name === 'Attendees') {
                    if (field.value.includes(`${interaction.user.username}`)) {
                        if (field.value.replace(`${interaction.user.username}`, '') === '') {
                            return { name: 'Attendees', value: 'Currently no attendees', inline: true };
                        }
                        return { name: 'Attendees', value: field.value.replace(interaction.user.username, ''), inline: true };
                    }
                }
                return field;
            });
